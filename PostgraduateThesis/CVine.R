...
udata <- read.csv("copula建模数据.csv")[, -1]

# 静态选择
# 2  2 14  1  2  0  4  0  0  1
CDVine::CDVineCopSelect(udata, familyset = c(1:6, 13, 14, 16), type = 1, indeptest = T)
...
for (i in 1:(d - 1)) {
  for (j in 1:(d - i)) {
    ...
    if (p > .05) {
      ...
      # 生成hessen矩阵
      reps <- 1e4
      R <- matrix(c(seq(-.99, -.5, length = 40), seq(-.51, .5, length = 20), seq(.51, .99, length = 40)), ncol = 1)
      ...
      if (rho0 > 1) {
        ...
        aic[6] <- 2 * 3 - 2 * (-joe_GAS_maxlik(theta[, 6], data, hessINFO6))
        if (aic_s[6] < aic[6]) {
          theta[, 6] <- c(w0, 0, 0)
          rho[, 6] <- rep(rho0, l)
          aic[6] <- aic_s[6]
  ...
  for (k in 1:(d - i)) {
    temp[, k] <- Hfunc(fam_end[i, k], V[, 1], V[, k + 1], rho_end[, k, i], nu_end[i, k])
  }
  V <- temp
}
##########################################plot###############################
